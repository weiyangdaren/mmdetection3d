...
class OmniDETR3D(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        super(OmniDETR3D, self).__init__(
    ...
    def add_lidar2img(self, batch_input_metas: List[Dict]) -> List[Dict]:
        ...
        for meta in batch_input_metas:
            l2i = list()
            for i in range(len(meta[self.img_key]['cam2img'])):
                c2i = torch.tensor(meta[self.img_key]['cam2img'][i]).double()
                l2c = torch.tensor(meta[self.img_key]['lidar2cam'][i]).double()
                l2i.append(get_lidar2img(c2i, l2c).float().numpy())
            meta[self.img_key]['lidar2img'] = l2i

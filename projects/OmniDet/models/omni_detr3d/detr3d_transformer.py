...
class Detr3DTransformer(BaseModule):
    ...
    def forward(self, mlvl_feats, query_embed, reg_branches=None, **kwargs):
        ...
        query_pos, query = torch.split(query_embed, self.embed_dims, dim=1)  # The query positional and semantic embedding
        query_pos = query_pos.unsqueeze(0).expand(bs, -1, -1)  # [bs,num_q,c]
        query = query.unsqueeze(0).expand(bs, -1, -1)  # [bs,num_q,c]
        reference_points = self.reference_points(query_pos)  # [bs,num_q,3]
...
class Detr3DCrossAtten(BaseModule):
    ...
    def forward(self,
        ...
        reference_points_3d, output, mask = feature_sampling(
            value, reference_points, self.pc_range, kwargs['img_metas'], input_key=self.input_key)
...
def feature_sampling(mlvl_feats,
    ...
    if not input_key:
        ...
        lidar2img = [meta[input_key]['lidar2img'] for meta in img_metas]
    ...
    (h, w) = img_metas[0]['pad_shape'] if not input_key else img_metas[0][input_key]['pad_shape']
    pt_cam[..., 0] /= w
    pt_cam[..., 1] /= h
    # else:
